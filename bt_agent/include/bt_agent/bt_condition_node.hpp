...
#ifndef NAV2_BEHAVIOR_TREE__BT_CONDITION_NODE_HPP_
#define NAV2_BEHAVIOR_TREE__BT_CONDITION_NODE_HPP_
...
class BtConditionNode : public BT::ConditionNode
{
public:
  BtConditionNode(
  ...
  : BT::ConditionNode(service_node_name, conf), service_name_(service_name)
  {
    ...
    RCLCPP_INFO(
      node_->get_logger(), "\"%s\" BtConditionNode initialized",
      service_node_name.c_str());
  }

  BtConditionNode() = delete;

  virtual ~BtConditionNode()
  {
  }

  // Any subclass of BtConditionNode that accepts parameters must provide a providedPorts method
  ...
  // method to check a returned value, for example.
...
protected:
  ...
  std::chrono::milliseconds server_timeout_{10000};
};

}  // namespace nav2_behavior_tree

#endif  // NAV2_BEHAVIOR_TREE__BT_CONDITION_NODE_HPP_
